 ...
 *
 * cd_read_sectors_ (0x8005f380, 0x1D8 bytes) — raw CD sector reader: reads
 * `length` bytes starting at byte `byteOffset` of sector `sector` into
 * `buffer` (see cd_read.c / cd_read_at_.c, both of which already declare
 * `extern void cd_read_sectors_(void *buffer, int sector, int byteOffset,
