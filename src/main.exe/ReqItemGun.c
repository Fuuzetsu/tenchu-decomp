 ...
 *  - instead of `item->model = (ModelType *)ItemImage[item->type];`, the tail
 *    block-copies the whole `p->end` VECTOR (4 words, align-4 word block
 *    move: 4x lw + 4x sw — see the matching cookbook's
 *    "cast type's alignment drives copy code" rule) onto PSX.SYM's
 *    `param_gun.vec`. The raw assembly confirms
