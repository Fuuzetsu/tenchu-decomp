...
compilerSdataPool src = pool (takeBaseName src)
  where
    ...
    pool "ProcItemNinken" = Just (CompilerSdataPool "__compiler_sdata_pool_y_n50" [0, -50, 0] False)
    pool _ = Nothing
...
maspsxGpExterns src = extra (takeBaseName src) <> concat [["--gp-extern", s] | s <- syms (takeBaseName src)]
  where
    ...
    extra "ACTION" = ["--expand-div"]
    extra "3DCTRL" = ["--expand-div"]
    extra "PADCMD" = ["--expand-div"]
    ...
    extra "CAMERA" = ["--expand-div"]
    extra "trace_ground_" = ["--expand-div"]
    extra "MISC" = ["--expand-div"]
    ...
    syms "3DCTRL" = ["GameClock", "SkipFrame", "DrawingPage", "PacketUsed", "OTablePt", "time"]
    syms "FILEIO" = ["AccessPower", "ReadMode", "TotalIO", "MemoryLoadAddress", "MDfat"]
    ...
    syms "CAMERA" = ["DEBUG_PAD_HELD_", "DEBUG_PAD_PRESS_", "DEBUG_CAMERA_INDEX_", "DEBUG_CAMERA_BASE_", "Projection"]
    syms "HUMAN" = ["Humans", "VISIBLE_ENEMIES_"]
    ...
    syms "GotoPosition" = ["Me_THINK_C", "Degree", "Attrib", "ProbeLevelLow"]
    ...
    syms "update_card_screen_" = ["McardState", "McardPage", "McardFile", "McardRetry", "McardStateFlag"]
    syms "update_card_message_" = ["McardRetryCount", "McardStateFlag"]
