...
except for the three layout fields that a normal link must regenerate and any
explicitly configured preserved-field overrides:

* initial PC (header offset 0x10),
* load address / ``t_addr`` (0x18), and
* sector-padded payload size / ``t_size`` (0x1c).

For example, ``--set sp=...`` lets the normal-link RAM policy own the initial
stack instead of silently inheriting the retail header template.
...
def finalize_image(
    ...
    *,
    ...
    for field, value in configured.items():
        require(field in HEADER_FIELDS, f"unknown PS-X EXE header field {field!r}")
        require(
            field not in ("pc", "t_addr", "t_size"),
            f"header field {field} is regenerated by the finalizer, not --set",
        )
        struct.pack_into(
            "<I",
            new_header,
    ...
    mutable = set()
    for field in ("pc", "t_addr", "t_size", *configured):
    ...
    for field, value in finalized_values.items():
...
def parse_field_values(
    ...
    for item in values:
        ...
        require(
            ...
            f"invalid {option} {item!r}; use FIELD=VALUE",
        )
        normalized = field.replace("-", "_").lower()
        field = FIELD_ALIASES.get(normalized, normalized)
        require(
            field in HEADER_FIELDS,
            f"unknown {option} field {field!r}; choose from {', '.join(HEADER_FIELDS)}",
        )
        value = parse_int(raw_value)
        if field in parsed:
            require(parsed[field] == value, f"conflicting {option} values for {field}")
        parsed[field] = value
    return parsed


def parse_expectations(values: Iterable[str]) -> dict[str, int]:
    return parse_field_values(values, option="--expect")
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    finalize.add_argument(
        ...
        metavar="FIELD=VALUE",
        help=(
            "set a preserved header field from normal-link policy; repeatable. "
            "PC/t_addr/t_size are always linker-derived"
        ),
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    overrides = (
        parse_field_values(args.set, option="--set")
