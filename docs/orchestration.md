...
| `tools/siblingdiff.py <Name> [<Sibling>]` | **the drafting kickstart for family functions** — auto-picks the nearest MATCHED sibling, prints its C plus a normalized asm diff (intra-function branches relativized to labels so the same shape ALIGNS; `ref:`=sibling, `tgt:`=target, identical runs collapsed). Transcribe the sibling's C, edit only the flagged instructions; a hunk differing in just registers = an allocation tie for rtldump/permute, not a source change. Turns a family draft from hand-tracing two `.s` files into a one-command diff. |
