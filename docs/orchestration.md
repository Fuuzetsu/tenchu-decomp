...
| `tools/stackplan.py <Name> [--emit-overlay]` | recover target/candidate frame size, outgoing arguments, saved-register boundary, and every stack access. `--emit-overlay` recognizes address-taken aggregates and prints a padded C workspace scaffold; use `--args N` to override an ambiguous inferred o32 boundary. |
