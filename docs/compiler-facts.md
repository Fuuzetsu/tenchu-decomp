...
- **Do not transcribe a strength-reduced array cursor back into C.** In a real
  ...
  lockstep exactly from indexing. All 22 scans go all the way back to direct
  ...
  fake identical-arm allocation fence. `SetBlood` likewise became exact only
  when direct indexing, its guarded outer infinite loop, and removal of an
  artificial inner one-shot wrapper were tested together. An explicit source
  scan pointer can hide
...
- **Frame slots are assigned in THREE phases**: `assign_parms` (addressable
  ...
  spill (`SetupTexScroll`'s exact source confirms vx/vy at the first two reload
