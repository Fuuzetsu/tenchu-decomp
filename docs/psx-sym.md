...
**48 names**, all rebuilt byte-identical, listed in `reference/psxsym-applied.tsv`.
...
* **`reference/psxsym-candidates.tsv`** — 3 names the TU alignment can still suggest
  after current repository names are overlaid. Every remaining row is `LOW`:
  positional evidence only. The current `ViewAdjustBG`, `GotoPosition`, and
  `RequestItem` suggestions remain unadopted. Earlier positional suggestions,
