...
**49 names**, all rebuilt byte-identical, listed in `reference/psxsym-applied.tsv`.
Eleven of them replaced *guessed* names with the originals, and the guesses corroborate
the recovery:
...
| `turn_towards_player_` | `GotoPosition` |
| `handle_char_state_dead_` | `ActDEAD` |
| `debug_menu_select_stage` | `SelectStage` |

`GotoPosition` is the deliberately adopted exception to the size-based LOW
classification. The demo and retail functions occupy the same slot between
`StateTransition` and `ChasetoTarget`, have the same two-long-argument contract,
and share the `GetDirection`/`Degree` turn-and-pad core. Retail grows that core
from 188 to 480 bytes by adding wall and terrain avoidance; callmatch covers the
demo callee set completely.

## What we did NOT get, and where it went

Three lists are regenerated by `tools/symmatch.py --candidates --unplaced --left`:

* **`reference/psxsym-candidates.tsv`** — 2 names the TU alignment can still suggest
  after current repository names are overlaid. Every remaining row is `LOW`:
  positional evidence only. The current `ViewAdjustBG` and `RequestItem`
  suggestions remain unadopted. Earlier positional suggestions,
  including `SetGore` for `0x80036284`, produced false positives in audits; the
  actual `SetGore` was recovered independently at `0x80035f44` from its allocator/
  callback relationship with `DrawGore`. A placeholder/current descriptive name
  stays until independent signature and semantic evidence identifies the function.

* **`reference/psxsym-unplaced.tsv`** — 44 functions PSX.SYM knows that we could not
...
* **`reference/psxsym-unnamed.tsv`** — 73 retail placeholders with no candidate at
