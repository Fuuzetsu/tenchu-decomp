...
retail `FUN_80027730`, while the currently named retail function launches a
...
That conservative Pareto test flags only `AttackFire` in the 45-name applied
control table; `FUN_80027730` is the strictly better competing fit.

## What we adopted

**45 names**, all rebuilt byte-identical, listed in `reference/psxsym-applied.tsv`.
...
| `handle_char_state_attacking_SEVEN_` | `AttackFire` (historical; allocation now contested) |
...
* **`reference/psxsym-unplaced.tsv`** — 46 functions PSX.SYM knows that we could not
  ...
  `AttackGunControl`, `PutMoji*`, `QuakeCamera` — are real game code and
